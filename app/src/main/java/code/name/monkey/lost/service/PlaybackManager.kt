...
class PlaybackManager(val context: Context) {
    ...
    fun maybeSwitchToCrossFade(crossFadeDuration: Int): Boolean {
        /* Switch to LostExoPlayer if CrossFade duration is 0 and
                Playback is not an instance of LostExoPlayer */
        if (playback !is LostExoPlayer && crossFadeDuration == 0) {
            if (playback != null) {
                playback?.release()
            }
            playback = null
            playback = LostExoPlayer(context)
            return true
    ...
    private fun createLocalPlayback(): Playback {
        // Set LostExoPlayer when crossfade duration is 0 i.e. off
        return if (PreferenceUtil.crossFadeDuration == 0) {
            LostExoPlayer(context)
        } else {
